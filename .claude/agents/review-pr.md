# review-pr — Bitbucket PR Reviewer
...
| Get PR activities | GET | `/rest/api/1.0/projects/{project}/repos/{repo}/pull-requests/{id}/activities` |
| Post comment | POST | `/rest/api/1.0/projects/{project}/repos/{repo}/pull-requests/{id}/comments` |
| List comments | GET | `/rest/api/1.0/projects/{project}/repos/{repo}/pull-requests/{id}/comments` |
...
- Otherwise, list open PRs and let the user choose, or review the most recent one.

### 2. Gather context

- Fetch the PR details (title, description, source branch, target branch).
- Fetch the full diff.
...
| **`engine/voting.py`** — Voting mechanisms | Partial | Only `majority` implemented, missing `weighted`, `consensus`, `human_in_loop` |
...
If no PR number is given, list open PRs and review the most recent one.
